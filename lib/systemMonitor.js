...
export class SystemMonitor extends PubSub {
    ...
    _readRam() {
        try {
            ...
            this._checkRamThreshold(percent);
            return {
    ...
    _checkRamThreshold(percent) {
        ...
        if (percent >= threshold && !this._aboveRamThreshold) {
            Main.notify(
                'RAM Warning',
                `RAM usage is at ${Math.round(percent)}%. Consider closing some applications.`
            );
            this._aboveRamThreshold = true;
