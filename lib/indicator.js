...
import { ContainersView } from './views/containersView.js';
import { CircularIndicator } from './utils/circularIndicator.js';
import { State } from './stopwatch.js';
import { State as PomodoroState } from './pomodoro.js';
...
class Indicator extends PanelMenu.Button {
    _init(extension) {
        ...
        this._pomodoro = null;
        this._pomodoroView = null;
        this._pomodoroSubId = null;
        this._pomodoroStateSubId = null;
        this._pomodoroPhaseSubId = null;
        ...
        this._panelPomodoroIcon = new St.Icon({
            gicon: Gio.icon_new_for_string(`${extension.path}/lib/icons/tomato-symbolic.svg`),
            style_class: 'system-status-icon',
            visible: false,
        });

        this._panelPomodoroTimer = new St.Label({
            text: '',
            y_align: Clutter.ActorAlign.CENTER,
        });
        ...
        this._panelBox.add_child(this._panelPomodoroIcon);
        this._panelBox.add_child(this._panelPomodoroTimer);
        this._panelBox.add_child(this._panelRamIcon);
        this._panelBox.add_child(this._panelRamLabel);
        this._panelBox.add_child(this._panelIcon);
        this.add_child(this._panelBox);

        // Fixed menu slots. Their visual order is not hardcoded here — it's
        // reapplied by _applyBlockOrder() right after all slots exist, driven
        // by the 'panel-blocks-order' setting (reorderable from Preferences),
        // so no separator is pinned between any two of them.
        ...
        // Pomodoro lives right below the stopwatch, in the same group.
        this._pomodoroSlot = new PopupMenu.PopupBaseMenuItem({ reactive: false, can_focus: false });
        this._pomodoroSlot.visible = false;
        this.menu.addMenuItem(this._pomodoroSlot);
        ...
        // Unlike the other slots, containers are real PopupSubMenuMenuItem
        // rows (GNOME's standard accordion menu item — same component the
        // system menu uses), so this holds a PopupMenuSection rather than a
        // plain placeholder: it's the only container type whose addMenuItem()
        // knows how to wire up a submenu's expand/collapse arrow correctly.
        this._containersSection = new PopupMenu.PopupMenuSection();
        this.menu.addMenuItem(this._containersSection);
        ...
        this.menu.connect('open-state-changed', (_menu, isOpen) => {
            ...
            if (isOpen && this._containersView)
                this._containersView.refresh();
        });

        this._settings = extension.getSettings();
        this._applyBlockOrder();
        this._blockOrderChangedId = this._settings.connect(
            'changed::panel-blocks-order', () => this._applyBlockOrder()
        );
    }

    // Menu items belonging to each reorderable block. System Monitor is two
    // slots (the rings + the RAM process list slot below it) that always
    // move together, immediately adjacent.
    _getBlockItems(key) {
        switch (key) {
            case 'stopwatch': return [this._stopwatchSlot];
            case 'pomodoro': return [this._pomodoroSlot];
            case 'system-monitor': return [this._monitorSlot, this._processSlot];
            case 'containers': return [this._containersSection];
            default: return [];
        }
    }

    // Reads 'panel-blocks-order' and repositions each block's menu item(s)
    // to match, via PopupMenuBase's own moveMenuItem() — any block missing
    // from the saved order (e.g. added in a later version) is appended at
    // the end rather than dropped.
    _applyBlockOrder() {
        const allKeys = ['stopwatch', 'pomodoro', 'system-monitor', 'containers'];
        const saved = this._settings.get_strv('panel-blocks-order').filter(k => allKeys.includes(k));
        const order = [...saved, ...allKeys.filter(k => !saved.includes(k))];

        let position = 0;
        for (const key of order) {
            for (const item of this._getBlockItems(key)) {
                this.menu.moveMenuItem(item, position);
                position++;
            }
        }
    }
    ...
    destroy() {
        if (this._blockOrderChangedId) {
            this._settings.disconnect(this._blockOrderChangedId);
            this._blockOrderChangedId = null;
        }
        super.destroy();
    }

    setupContainers(settings) {
        this._containersView = new ContainersView(settings, this._containersSection);
    }

    teardownContainers() {
        if (this._containersView) {
            this._containersView.destroy();
            this._containersView = null;
        }
        this._containersSection.removeAll();
    }
    ...
    setupStopwatch(stopwatch) {
        ...
        // Reflect a state restored from GSettings (e.g. after a Shell
        // reload) immediately, instead of waiting for the next tic.
        if (this._stopwatch.state !== State.RESET) {
            const time = this._fmtHms(this._stopwatch.total);
            this._panelStopwatchIcon.visible = true;
            this._panelStopwatchIcon.accessible_name = `Stopwatch: ${time}`;
            this._panelTimer.set_text(time);
        }
    ...
    setupPomodoro(pomodoro) {
        this._pomodoro = pomodoro;

        this._pomodoroView = new PomodoroView(pomodoro, () => this._openPrefsAt('pomodoro'));
        this._pomodoroSlot.add_child(this._pomodoroView.actor);
        this._pomodoroSlot.visible = true;

        const updatePanel = () => {
            if (this._pomodoro.state === PomodoroState.RESET) {
                this._panelPomodoroTimer.set_text('');
                this._panelPomodoroIcon.visible = false;
                return;
            }

            const time = this._fmtMinSec(this._pomodoro.remaining);
            this._panelPomodoroIcon.visible = true;
            this._panelPomodoroIcon.accessible_name = `Pomodoro: ${time}`;
            this._panelPomodoroTimer.set_text(time);
        };

        this._pomodoroStateSubId = this._pomodoro.subscribe('state_change', updatePanel);
        this._pomodoroPhaseSubId = this._pomodoro.subscribe('phase_change', updatePanel);
        this._pomodoroSubId = this._pomodoro.subscribe('tic', updatePanel);
    ...
    _openPrefsAt(page) {
        this._extension.getSettings().set_string('prefs-open-page', page);
        this._extension.openPreferences();
    }

    teardownPomodoro() {
        if (this._pomodoroStateSubId && this._pomodoro) {
            this._pomodoro.unsubscribe(this._pomodoroStateSubId);
            this._pomodoroStateSubId = null;
        }
        if (this._pomodoroPhaseSubId && this._pomodoro) {
            this._pomodoro.unsubscribe(this._pomodoroPhaseSubId);
            this._pomodoroPhaseSubId = null;
        }
        if (this._pomodoroSubId && this._pomodoro) {
            this._pomodoro.unsubscribe(this._pomodoroSubId);
            this._pomodoroSubId = null;
        }
        if (this._pomodoroView) {
            this._pomodoroView.destroy();
            this._pomodoroView = null;
        }
        this._panelPomodoroTimer.set_text('');
        this._panelPomodoroIcon.visible = false;
