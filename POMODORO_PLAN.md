...
### 12. Notificações, som e aviso de "quase acabando" (adicionado após feedback do usuário)
- [x] Chaves novas: `pomodoro-notify-enabled` (b, default true), `pomodoro-sound-enabled` (b,
      default true), `pomodoro-warn-enabled` (b, default false), `pomodoro-warn-minutes` (i,
      default 1, range 1–30)
- [x] `pomodoro.js`: `Main.notify()` + `global.display.get_sound_player().play_from_theme()` ao
      concluir uma fase naturalmente (`alarm-clock-elapsed`) — nunca no `skip()` (ação deliberada,
      sem necessidade de aviso) nem no `_catchUp()` (fases perdidas enquanto o Shell estava fechado
      não geram notificação retroativa)
- [x] Aviso de "faltam N minutos" (`bell`), disparado no máximo uma vez por fase (`_warned`,
      resetado em `_advance()`); ao restaurar estado após reload, calculado sem re-disparar se já
      estava dentro da janela de aviso
- [x] `prefs.js`: grupo "Notifications" na página Pomodoro com os 4 controles acima
